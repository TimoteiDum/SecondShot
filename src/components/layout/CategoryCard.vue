<template>
  ...
  <div class="w-full flex items-center justify-center mb-4">
      <div :class="['w-20 h-20 rounded-full flex items-center justify-center', circleBgClass()]">
        ...
        <i v-else :class="iconClass + ' icon-display'" :style="{ color: iconColor() }" aria-hidden="true"></i>
      </div>
    </div>
...
function iconColor() { return props.theme === 'blue' ? '#0284c7' : '#f97316' }
</script>

<style scoped>
.bi { font-size: 28px; }
.icon-display { font-size: 36px; color: #f97316; margin-bottom: 0.25rem; }
