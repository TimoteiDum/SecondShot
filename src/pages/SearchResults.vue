    ...
    <div class="max-w-6xl mx-auto text-center relative z-10">
      ...
      <div v-else>
        ...
        <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          <ProductCard v-for="p in products.filteredProducts" :key="p.id" :product="p" @add-to-cart="onAddToCart" @view-details="onViewDetails" />
        </div>
      </div>
    </div>
  </div>
...
function onAddToCart(product) {
  ...
  try { ui.showToast(product && product.title ? product.title : '') } catch (e) { }
}
...
.cameras-hero-bg {
  position: relative;
  overflow: hidden;
  background: linear-gradient(90deg, rgba(30,58,138,0.18) 0%, rgba(37,99,235,0.14) 50%, rgba(14,165,233,0.12) 100%);
}
.cameras-hero-bg::before {
  content: '';
  position: absolute;
  inset: -22% -18% -28% -18%;
  background: radial-gradient(circle at 12% 18%, rgba(99,102,241,0.30), transparent 22%),
