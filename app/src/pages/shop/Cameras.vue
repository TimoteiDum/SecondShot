    ...
    <main class="min-h-screen pt-24 px-6 cameras-hero-bg">
      <div class="max-w-4xl mx-auto py-8 relative z-10">
        <div class="mb-6 text-center">
          <h1
            class="text-5xl sm:text-6xl font-extrabold leading-tight bg-clip-text text-transparent bg-gradient-to-r from-orange-400 via-orange-600 to-red-600 drop-shadow-lg"
          >
            Cameras
          </h1>
          <p
            class="mt-3 text-lg sm:text-xl font-medium bg-clip-text text-transparent bg-gradient-to-r from-orange-300 via-orange-500 to-red-500/90"
          >
        ...
        <div
          class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-6 items-stretch"
        >
          <ProductCard
            v-for="product in products.byCategory('Cameras').slice(0, 3)"
            :key="product.id"
            :product="product"
          ...
          <div v-if="showDetails" class="fixed inset-0 z-60 flex items-center justify-center px-4">
            ...
            <div class="relative details-modal max-w-2xl w-full z-20 text-white">
              <div class="flex items-start gap-4">
                <img
                  :src="lastViewedProduct.image || '/camera.jpg'"
                  alt=""
                  class="w-40 h-28 object-contain object-center rounded-md bg-black/6 p-1"
                />
                <div class="flex-1">
                  ...
                  <div class="mt-4 flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-3">
                      <button
                        @click="addToCart(lastViewedProduct)"
                        class="bg-gradient-to-r from-orange-400 via-orange-600 to-red-600 text-white px-4 py-2 rounded-md"
                      >
                        Add to cart
                      </button>
                      <button
                        @click="closeDetails"
                        class="bg-white/6 text-white px-4 py-2 rounded-md"
                      >
                        Close
                      </button>
                    </div>
...
function addToCart(p) {
  ...
  try {
    ui.showToast(p && p.title ? p.title : '')
  } catch (e) {}
}
...
.cameras-hero-bg::before {
  ...
  inset: -25% -20% -30% -20%;
  background:
    radial-gradient(circle at 10% 10%, rgba(251, 146, 60, 0.28), transparent 24%),
...
.cameras-hero-bg > .max-w-4xl {
  z-index: 10;
  position: relative;
}

.cameras-hero-bg .bg-white\/6 {
  background-color: rgba(10, 8, 6, 0.12) !important;
