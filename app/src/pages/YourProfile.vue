<template>
  <div
    class="min-h-screen flex items-start justify-center pt-24 px-4 bg-cover bg-center relative"
    style="background-image: url('/background.jpg')"
  >
    <div class="absolute inset-0 bg-black/40 pointer-events-none"></div>
    <div
      class="w-full max-w-4xl profile-panel p-6 grid grid-cols-1 md:grid-cols-4 gap-6 relative z-10"
    >
          ...
          <div v-if="active === 'account'">
            ...
            <p>
              <router-link
                ...
                style="color: #c7b3ff"
                >Manage your account details such as name, email and shipping address.</router-link
              >
            </p>
      ...
      <button
        @click="handleLogout"
        class="hidden md:inline-flex fixed bottom-6 right-6 bg-red-600 hover:bg-red-700 text-white px-4 py-3 rounded-full shadow-lg items-center"
        aria-label="Log out"
      >
      ...
      <router-link
        to="/"
        class="hidden md:inline-flex fixed bottom-6 left-6 bg-sky-500 hover:bg-sky-600 text-white px-4 py-3 rounded-full shadow-lg items-center"
        aria-label="Home"
      >
...
function handleLogout() {
  ...
  const base =
    import.meta && import.meta.env && import.meta.env.BASE_URL ? import.meta.env.BASE_URL : '/'
...
.profile-panel .fixed[aria-label='Log out'] {
  background: linear-gradient(90deg, #ef4444, #dc2626);
}
.profile-panel .fixed[aria-label='Home'] {
