<template>
  <div class="bg-white/6 backdrop-blur rounded-xl p-4 shadow-md flex flex-col h-full min-h-72">
    <img :src="product.image || '/camera.jpg'" alt="" class="w-full h-40 object-contain object-center rounded-md mb-4 bg-black/6 p-2" />
    <h2 class="text-xl sm:text-2xl font-semibold text-white">{{ product.title }}</h2>
    <p class="text-sm sm:text-base text-gray-300 mt-2 mb-4 flex-1">{{ product.description || 'Great camera for photography.' }}</p>
